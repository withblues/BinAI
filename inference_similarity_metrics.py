...
if __name__ == "__main__":
    ...
    all_files = [
        ...
        if f.endswith("-results-cosine.csv")
    ]
    ...
    merged_df["ground_truth"] = (
        merged_df["anchor_function_name"] == merged_df["target_function_name"]
    ).astype(int)
    ...
    for _, group in tqdm(merged_df.groupby(group_keys), desc="calculating metrics ..."):
        ...
        for sim_col in sim_columns:
            ...
            metrics[f"NDCG@{top_k}_{model}"].append(
                normalized_dcg(ground_truth, student_scores, top_k)
            )
            metrics[f"Precision@{top_k}_{model}"].append(
                precision_at_k(sorted_relevance, top_k)
            )
