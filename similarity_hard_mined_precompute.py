...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--top_k", type=int, default=10, help="Total targets per anchor"
    )
    parser.add_argument(
        "--batch_size", type=int, default=2048, help="Anchors to process per batch"
    )
    parser.add_argument(
        "--num_candidates_faiss",
        type=int,
        default=512,
        help="Nearest neighbors to retrieve from FAISS",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Min score to be a hard positive",
    )
    parser.add_argument(
        "--hard_negative_max_score",
        type=float,
        default=0.7,
        help="Max score for a hard negative",
    )
    parser.add_argument(
        "--hard_negative_min_score",
        type=float,
        default=0.2,
        help="Min score for a hard negative",
    )
    args = parser.parse_args()

    assert (
        args.num_hard_positives + args.num_hard_negatives + args.num_random_negatives
        == args.top_k
    )

    # load precomputed teacher embeddings
    teacher_embeddings_dict = load_data(
        os.path.join(args.data_dir, "clap/datasets", f"{args.split}-embeddings.pkl")
    )
    ...
    for start_idx in tqdm(
        range(0, total_embeddings, args.batch_size), desc="Processing anchors"
    ):
        ...
        D, I = index.search(
            batch_anchor_embeddings, args.num_candidates_faiss + 1
        )  # + 1 to include self
        ...
        for i, anchor_global_idx in enumerate(batch_anchor_indices):
            ...
            if num_random_to_add > 0:
                ...
                random_indices = np.random.choice(
                    total_embeddings, size=num_random_to_add * 2, replace=False
                )
                ...
                for rand_idx in random_indices:
                    if added_count >= num_random_to_add:
                        break
                    if (
                        rand_idx != anchor_global_idx
                        and int(keys[rand_idx]) not in current_target_keys
                    ):
    ...
    output_path = os.path.join(
        args.output_dir, "clap/datasets", f"{args.split}-function-pool-hard_mined"
    )
