...
if __name__=="__main__":
    ...
    # load dataset
    train_dataset_anchor = load_from_disk(os.path.join(data_dir, 'clap-train-function-pool'))
    train_dataset_tokenized = load_from_disk(os.path.join(data_dir, 'distil-train-tokenized'))
    valid_dataset_anchor = load_from_disk(os.path.join(data_dir, 'clap-valid-function-pool'))
    valid_dataset_tokenized = load_from_disk(os.path.join(data_dir, 'distil-valid-tokenized'))
    ...
    def create_collate_fn(dataset_tokenized):
        def collate_fn(batch):
            ...
            padded = pad_sequence(list(id_to_tensor.values()), batch_first=True, padding_value=0)
    ...
    # dataloader
    train_dataloader = DataLoader(train_dataset_anchor, batch_size=batch_size, collate_fn=create_collate_fn(train_dataset_tokenized), shuffle=True, num_workers=4)
    valid_dataloader = DataLoader(valid_dataset_anchor, batch_size=batch_size, collate_fn=create_collate_fn(valid_dataset_tokenized), shuffle=False, num_workers=4)
