...
class DistillTrainer:
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        for i, batch_dict in enumerate(data_iter):
            student_all_parts_batched = batch_dict["student_instruction"].to(
                self.device
            )
            attention_mask_batched = batch_dict["student_attention_mask"].to(
                self.device
            )
            teacher_embeddings_batch = batch_dict["teacher_embedding"].to(self.device)

            current_dataloader_batch_size, dynamic_max_len, _ = (
                student_all_parts_batched.shape
            )
            with torch.set_grad_enabled(train):
                ...
                reshaped_input_for_encode = student_all_parts_batched.view(
                    current_dataloader_batch_size * dynamic_max_len, self.total_seq_len
                )
                ...
                encoded_parts_batched_view = encoded_all_parts.view(
                    current_dataloader_batch_size, dynamic_max_len, 128
                )

                # masked summation so we discard padded instructions
                attention_mask_expanded = attention_mask_batched.unsqueeze(-1)
                masked_encoded_parts = (
                    encoded_parts_batched_view * attention_mask_expanded
                )
                student_summed_function_embeddings = torch.sum(
                    masked_encoded_parts, dim=1
                )

                # mean pooling
                valid_counts = attention_mask_expanded.sum(dim=1).clamp(min=1e-5)
                student_mean_function_embeddings = (
                    student_summed_function_embeddings / valid_counts
                )

                # normalize student embeddings
                student_mean_function_embeddings = F.normalize(
                    student_mean_function_embeddings, dim=1
                )

                # project teacher embeddings and calculate loss
                projected_teacher_embeddings = self.projector(teacher_embeddings_batch)
                loss = self.criterion(
                    student_mean_function_embeddings, projected_teacher_embeddings
                )
...
if __name__ == "__main__":
    ...
    def collate_fn(batch, pad_token_id):
        ...
        teacher_embeddings = torch.stack(
            [sample["teacher_embedding"] for sample in batch]
        )
    ...
    train_dataloader = DataLoader(
        train_dataset_combined, batch_size=batch_size, collate_fn=collate, shuffle=True
    )
    valid_dataloader = DataLoader(
        valid_dataset_combined, batch_size=batch_size, collate_fn=collate
    )
    ...
    if gpu_monitor.memory_usage:
        ...
        print(
            f"Average Memory Usage: {gpu_monitor.get_memory_usage(average=True):.2f} MB"
        )
    if gpu_monitor.utilization:
        print(f"Peak Utilization: {gpu_monitor.get_utilization(peak=True) * 100:.2f}%")
        print(
            f"Average Utilization: {gpu_monitor.get_utilization(average=True) * 100:.2f}%"
        )
    print("-------------------------")
    writer.close()
