...
if __name__=="__main__":
    ...
    bert_model.load_state_dict(torch.load(os.path.join(data_dir, 'outputs', f'baseline-model')))
    bert_model.to(args.device)
    bert_model.eval()
    sum = 0
    count = 0
    for chunk in pd.read_csv(csv_path, chunksize=args.chunksize, dtype={"anchor_version": str, "target_version": str}):
        ...
        for (a_name, a_compiler, a_ver, a_opt, a_bin) in function_pools:
            if((a_name, a_compiler, a_ver, a_opt, a_bin) not in exist_embeddings):
                try:
                    ...
                    print(f"Warning: missing anchor function: {(a_name, a_compiler, a_ver, a_opt, a_bin)}")
                    continue
                a_embedding = get_bert_output(a_assembly, bert_model, tokenizer)
                exist_embeddings[(a_name, a_compiler, a_ver, a_opt, a_bin)] = a_embedding
                if(count==0):
                    print((a_name, a_compiler, a_ver, a_opt, a_bin))
                    print(a_embedding)
            else:
                a_embedding = exist_embeddings[(a_name, a_compiler, a_ver, a_opt, a_bin)]
            for (t_name, t_compiler, t_ver, t_opt, t_bin) in function_pools[(a_name, a_compiler, a_ver, a_opt, a_bin)]:
                if((t_name, t_compiler, t_ver, t_opt, t_bin) not in exist_embeddings):
                    try:
                        t_assembly = test_data[(t_name, t_compiler, t_ver, t_opt, t_bin)]
                    except KeyError:
                        print(f"Warning: missing target function: {(t_name, t_compiler, t_ver, t_opt, t_bin)}")
                        continue
                    ...
                    if(count==0):
                        ...
                        print(t_embedding)
                        count += 1
                    exist_embeddings[(t_name, t_compiler, t_ver, t_opt, t_bin)] = t_embedding
                else:
                    t_embedding = exist_embeddings[(t_name, t_compiler, t_ver, t_opt, t_bin)]
        ...
        output_temp.to_csv(output_csv_path, mode='a', header=not os.path.exists(output_csv_path), index=False)
        sum += len(results)
