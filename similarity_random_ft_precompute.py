...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate a random baseline dataset for Fine-Tuning with exactly 1 positive and k-1 negatives.")
    ...
    # --- 1. Setup Phase ---
    # ... (This part is identical and correct) ...
    ...
    for data_split in ["train", "val"]:
        ...
        # --- Create the Split's Universe (Identical and correct) ---
        ...
        split_dataset.set_format("numpy", columns=['unique_id', 'binary_name', 'function_name'])
        ...
        split_gt_lookup = {}
        for uid, b_name, f_name in tqdm(zip(split_data_np['unique_id'], split_data_np['binary_name'], split_data_np['function_name']), desc=f"Building {data_split} GT lookup"):
        ...
        # --- MINE PAIRS WITHIN THIS SPLIT'S UNIVERSE ---
        anchor_ids_for_split = split_ids_by_group[data_split]
        # OPTIMIZATION: Create this once. It's just a list of numbers.
        ...
        for anchor_id in tqdm(anchor_ids_for_split, desc=f"Mining {data_split} pairs"):
            ...
            anchor_meta = {'binary_name': split_dataset[anchor_idx_in_split]['binary_name'], 'function_name': split_dataset[anchor_idx_in_split]['function_name']}
    ...
    if not final_anchor_ids:
        ...
        final_ds = Dataset.from_dict({ "unique_id": final_anchor_ids, "positive_ids": final_positive_ids, "negative_ids": final_negative_ids, "split": final_splits })
        print(f"\nFinal dataset stats:\n{final_ds}")
