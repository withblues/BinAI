...
if __name__ == "__main__":
    ...
    # build key mapper
    teacher_dict = {
        make_key(row): row["embedding"]
        for row in tqdm(teacher_dataset, desc="build key mapper...")
    }
    ...
    reduced_teacher_dict = {
        k: reduced_embeddings[i]
        for i, k in tqdm(
            enumerate(keys), desc="build reduced key mapper", total=len(teacher_dict)
        )
    }
    ...
    for model_name, student_dataset in tqdm(
        zip(model_names, student_datasets),
        desc="calculating metrics for each student...",
        ...
        for row in student_dataset:
            ...
            if key in reduced_teacher_dict:
                if model_name == "distil_projected":
                    ...
                    metrics[f"Cosine_{model_name}"].append(
                        np.dot(row["embedding"], teacher_dict[key])
                    )
                    metrics[f"MSE_{model_name}"].append(
                        mean_squared_error(student_embedding, teacher_embedding)
                    )
                    metrics[f"MAE_{model_name}"].append(
                        mean_absolute_error(student_embedding, teacher_embedding)
                    )
                    ...
                    metrics[f"Cosine_{model_name}"].append(
                        np.dot(row["embedding"], reduced_teacher_dict[key])
                    )
                    metrics[f"MSE_{model_name}"].append(
                        mean_squared_error(student_embedding, teacher_embedding)
                    )
    ...
    for metric_name in metrics_to_plot:
        ...
        bars = plt.bar(
            models_for_plot, values_for_plot, color=colors_for_plot, edgecolor="black"
        )
