...
if __name__ == '__main__':
    ...
    # load csv 
    df = pd.read_csv(os.path.join(data_dir, 'function_pools.csv'), dtype={"anchor_version": str, "target_version": str})
    df_grouped = df.groupby([
        "anchor_function_name", "anchor_compiler", "anchor_version",
        "anchor_opt", "anchor_function_bin"
    ])
    ...
    for anchor_key, group_df in tqdm(df_grouped, desc='processing anchors...'):
        ...
        for _, row in group_df.iterrows():
            target_key = (
                row["target_function_name"], row["target_compiler"], row["target_version"],
                row["target_opt"], row["target_function_bin"]
            )
        ...
        for target_info, sim_score in zip(target_keys, similarities):
            ...
            results.append({
                "anchor_function_bin": a_bin, "anchor_function_name": a_name,
                "anchor_compiler": a_compiler, "anchor_version": a_ver, "anchor_opt": a_opt,
                "target_function_bin": t_bin, "target_function_name": t_name,
                "target_compiler": t_compiler, "target_version": t_ver, "target_opt": t_opt,
