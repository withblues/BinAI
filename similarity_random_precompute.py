...
if __name__ == "__main__":
    ...
    data = load_data(
        os.path.join(data_dir, "clap/datasets", f"{args.split}-embeddings.pkl")
    )
    ...
    for start_idx in tqdm(
        range(0, total, args.batch_size), desc="calculating similarity ..."
    ):
    ...
    output_path = os.path.join(
        output_dir, "clap/datasets", f"{args.split}-function-pool"
    )
