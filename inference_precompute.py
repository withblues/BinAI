...
if __name__ == "__main__":
    ...
    print(f"inference on {model_type} model")
    ...
    for _, row in tqdm(df.iterrows(), desc="collecting unique keys ...", total=len(df)):
    ...
    for key in tqdm(unique_keys, desc="creating batch ..."):
        ...
        if model_type == "clap":
            formatted_instruction = {
                str(i + 1): inst.strip() for i, inst in enumerate(asm)
            }
        else:
    ...
    output_dir = os.path.join(output_dir, "inference/datasets")
    os.makedirs(output_dir, exist_ok=True)

    metadata_file_path = os.path.join(output_dir, f"{model_type}-metadata.json")

    all_runs_data = []

    # load old data if there
