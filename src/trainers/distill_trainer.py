...
class DistillTrainer(BaseTrainer):
    def __init__(
        ...
        print(
            f"Distill Trainer - Trainable Projector Parameters: {trainable_params_proj}"
        )
        print(
            f"Distill Trainer - Total Trainable Parameters: {trainable_params_bert + trainable_params_proj}"
        )
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        for i, batch_dict in enumerate(data_iter):
            ...
            with torch.set_grad_enabled(train):
                ...
                projected_student_embeddings = self.projector(
                    student_mean_function_embeddings
                )
                projected_student_embeddings = F.normalize(
                    projected_student_embeddings, dim=1
                )

                loss = self.criterion(
                    projected_student_embeddings, teacher_embeddings_batch
                )
